...
from .errors import *
from ..SocketPlus import AsyncSocket
from ..SocketStructure.SocketGroup import AsyncSocketGroup, SocketConnect
...
class SocketBroadcastRoomClient:
    ...
    async def connect(self, host, port, timeout=None):
        ...
        await self.sock.join()

        res = await self.sock.read(timeout)
        if res is None:
