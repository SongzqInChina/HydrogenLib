...
class BinStruct:
    ...
    方法:
        ...
        - unpack(data) **不可覆写** 解包结构体,返回一个以cls为基类的新结构体实例(新返回的结构体实例的类型与打包时的结构体类型会不一致)
            比如:
                ...
                ```
            虽然类型名称一样,但是实际类型不一样
            原因可查看源码(/src/HydrogenLib/HyStruct/S_BinStruct.py)
    属性:
        - _data_ 需要打包的属性**列表**
    """
    ...
    def pack(self):
        ...
        for name in self._data_:
            ...
            if isinstance(origin_data, BinStruct):
                ...
                packed_data = BaseStruct.pack(origin_data)
                type_name = TypeFunc.get_type_name(origin_data)
            length_1 = len(name)
    ...
    @final
    def unpack(cls, data):
        offset = TypeFunc.IndexOffset.Offset(data)
        this_length = TypeFunc.bytes_to_int(offset > 1)
        ...
        while not offset.isend():
            ...
            if type_name == 'BinStruct':
                ...
                origin_data = BaseStruct.unpack(getattr(builtins, type_name), packed_data)
