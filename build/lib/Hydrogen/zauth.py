...
from . import zdatabase, typefunc, zhash
...
class SimpleUserPasswdManager:
    ...
    def add(self, username: str, passwd: str, hash_type="sha256", hash_lenght=64):
        ...
        passwd_hash = zhash.gethashByName(passwd_salt, hash_type, hash_lenght)
    ...
    def query(self, username, passwd):
        try:
            ...
            if zhash.gethashByName(user_passwd_salt, usr["hash_type"], usr["hash_lenght"]) == true_passwd_hash:
                return True
            else:
                return False
...
class JsonWebToken(TokenBase):
    ...
    def _generate_signature(self):
        ...
        # 计算 HMAC-SHA256 签名
        signature = hmac.new(self.secret.encode(), msg=message, digestmod=zhash.hashlib.sha256).digest()
    ...
    def deserialize(cls, secret, token_str: str) -> 'JsonWebToken':
        ...
        expected_signature = hmac.new(secret.encode(),
                                      msg=f"{base64.urlsafe_b64encode(json.dumps(head).encode()).decode()}.{base64.urlsafe_b64encode(json.dumps(payload).encode()).decode()}".encode(),
                                      digestmod=zhash.hashlib.sha256).digest()
...
class HashToken(TokenBase):
    def __init__(self, original_token=None, salt=None):
    ...
    def serialize(self):
        return zhash.gethashByName(self.original_token + self.salt, "sha256")

    @classmethod
    def deserialize(cls, *args, **kwargs):
        raise Exception("无法反序列化SimpleToken，因为此类不能被反序列化。")
...
class HashTokenManager:
    ...
    def add_token(self, original_token=None, salt=None):
